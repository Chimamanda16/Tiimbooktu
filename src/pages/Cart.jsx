...
export const Cart = () => {
    const initialData = [
        {
            img: '/assets/images/sample-cart.png',
            title: "Mosaic, or Q",
            amount: 11,
            quantity: 1,
        },
    ...
    const totalAmount = cartData.reduce((acummulator, item) => {
        return acummulator + ( item.amount * item.quantity)
    }, 0).toFixed(2)
    ...
    const decreaseQuantity = (index) => {
        setCartData(prev =>
          prev.map((item, i) => 
            i === index ? { ...item, quantity: item.quantity >= 1 ? item.quantity - 1 : 0 } : item
          )
        );
      };
            ...
            >
                <div className="flex flex-col w-full lg:items-center gap-4 lg:gap-8 mb-6">
                    <div className="flex flex-col gap-6 w-full">
                        ...
                        {cartData.length <= 0 && <EmptyDataState img="empty-cart" title="Your Cart is empty" description="your cart is empty at the moment, Explore our shop below" />}
                        {cartData.length > 0 && <div className="grid lg:grid-cols-3 gap-6 items-start w-full">
                                        ...
                                        return (  <div key={index} className="bg-[#2B2B2B] flex items-start justify-between p-3 pr-5 border border-[#595959] rounded-[20px] w-full">
                                            <div className="flex gap-10 lg:gap-3">
                                                ...
                                                <div className="w-[130px] h-[117px] md:w-[213px] md:h-[189px] rounded-[14px] overflow-hidden">
                                                    <img className="w-fit" src={data.img} alt="cart" />
                                                </div>
                                                <img className="cursor-pointer w-6 h-6 block md:hidden" src="/assets/icons/delete-icon-white.svg" alt="delete" />
                                                </div>
                                                <div className="flex flex-col justify-between">
                                                    <div className="flex flex-col gap-3 lg:gap-2">
                                                        <h6 className="text-white text-xl capitalize">{data.title} </h6>
                                                        <span className="text-xl font-bold text-white">${data.amount}</span>
                                                    </div>
                                                    <div className="text-white flex items-center">
                                                        <button onClick={() => decreaseQuantity(index)} className="border-r border-[#595959] py-4 px-5 bg-[#0A0A0A] rounded-l-[12px]"> - </button>
                                                        <div className="py-4 px-5 bg-[#0A0A0A] font-cinzel"> {data.quantity} </div>
                                                        <button onClick={() => increaseQuantity(index)} className="border-l border-[#595959] py-4 px-5 bg-[#0A0A0A] rounded-r-[12px]"> + </button>
                                                    </div>
                                                </div>
                                            </div>
                                            <img className="cursor-pointer hidden md:flex" src="/assets/icons/delete-icon-white.svg" alt="delete" />
                                        </div>)
                                    })}  
                                    </div>
