  ...
  const guestnetnoData = [
    {
      ...
      date: "January 3, 2025",
      title:
        "How to Fall in Love (Without Ever Admitting that is What You are Doing.)",
      ...
      >
        <div className="flex flex-col w-full lg:items-center gap-4 lg:gap-[123px] mb-6">
          <div className="flex flex-col gap-2 lg:text-center">
            <h1 className="lg:text-[81px] text-[24px] font-chango text-white lg:text-center uppercase">
              GuesTnetno
            </h1>
            <p className="font-glacial text-xl text-[#D9D9D9]">
              “It’s sorta like, we write what we want. Not what they like.”{" "}
              <br /> _Thee Guest.
            </p>
          </div>
        ...
        <div className="grid lg:grid-cols-3 w-full gap-5 font-glacial">
          {guestnetnoData.map((data, index) => {
            return (
              <div
                key={index}
                className="bg-white py-4 px-5 flex flex-col gap-[14px] uppercase"
              >
                ...
                <div className="flex justify-center">
                  <Link to={`/guest/${data.id}`}>
                    <button className="bg-[#CDFFAD] rounded-[35px] py-4 px-8 font-bold text-[#1C1C1C] uppercase">
                      Read more
                    </button>
                  </Link>
                </div>
