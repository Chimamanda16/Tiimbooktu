    ...
    <nav className="relative mb-[10px] text-[#fff] font-cinzel">
      ...
      <div className="flex justify-between items-center border-b-2 border-[#353535] px-[5%] py-[2%] max800:hidden">
        <div className="flex gap-6">
          <div className="flex gap-2 relative capitalize">
            Tiimbooktu
            {tiimbooktuMenu ? (
              <img className="rotate-180" onClick={() => toggleSubMenu('tiimbooktuMenu')} src="/assets/icons/nav-arrow-down.svg" alt="" />
            ) : (
              <img onClick={() => toggleSubMenu('tiimbooktuMenu')} src="/assets/icons/nav-arrow-down.svg" alt="" />
            )}
            {tiimbooktuMenu && (
          ...
          <div className="flex gap-2 relative capitalize">
            Content
            {contentMenu ? (
              <img className="rotate-180" onClick={() => toggleSubMenu('content')} src="/assets/icons/nav-arrow-down.svg" alt="" />
            ) : (
              <img onClick={() => toggleSubMenu('content')} src="/assets/icons/nav-arrow-down.svg" alt="" />
            )}
            {contentMenu && (
              <div className="absolute top-8 w-[250px] flex flex-col gap-4 left-0 border-[2px] border-[#595959] rounded-[20px] bg-[#0A0A0A] p-3">
      ...
      {menuOpen && (
        <div className="flex lg:hidden flex-col border-[2px] mt-2 mx-[1%] border-[#595959] rounded-[20px] gap-4 bg-[#0A0A0A] px-[5%] py-6 max800:flex">
          ...
          {isLoggedIn && <>
            <button onClick={ () => logOut()}
                    className="relative flex font-cinzel items-center justify-center text-[15px] bg-[#c53636] text-[#fff] font-normal rounded-[12px] p-2"
                    disabled={loading}
                  >
                    {loading ? (
                      <>
