...
export const RelatedProduct = () => {
    ...
    return <div className="flex flex-col gap-8 w-[88%] mx-auto text-white">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 justify-center text-center gap-4 max-lg:grid-cols-2 max-sm:grid-cols-2 mb-8 lg:mb-16">
            {relatedData.map((data, index) => {
                return <div key={index} className="border-[#353535] border-[1px] p-2 font-glacial flex flex-col gap-2 items-center">
                    <img className="w-[100%]" src={data.img} alt="Alt title" />
                    <h3 className="">{data.title}</h3>
                    <p className="text-[20px] font-400">${data.amount}</p>
                    <button className="p-2 border-[#fff] border-[1px] text-[15px] md:text-[20px] mt-2">Add To Cart</button>
                </div>
            })}
        </div>
    </div>
